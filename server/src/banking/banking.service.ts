...
import { BankingAnalysisRecord, BankingReprocessRequest } from "./banking.types";

export class BankingService {
  private processor: OcrProcessor;
  private analyzer: BankingAnalyzer;
  ...
  async analyze(request: BankingReprocessRequest): Promise<BankingAnalysisRecord> {
    ...
    const allTransactions = [] as ReturnType<BankingAnalyzer["parseTransactions"]>;
    ...
    return record as unknown as BankingAnalysisRecord;
  }
  ...
  async listByApplication(applicationId: string) {
    ...
    return records as unknown as BankingAnalysisRecord[];
  }
