import { BankingMetrics, BankingTransaction } from "./banking.types";

export class BankingAnalyzer {
  parseTransactions(rawText: string): BankingTransaction[] {
  ...
  categorize(transactions: BankingTransaction[]) {
  ...
  computeMetrics(transactions: BankingTransaction[]): BankingMetrics {
    const byMonth = new Map<string, BankingTransaction[]>();
  ...
  monthlyBreakdown(transactions: BankingTransaction[]) {
    const breakdown: Record<string, BankingTransaction[]> = {};
    transactions.forEach((t) => {
      const month = t.date.slice(0, 7);
