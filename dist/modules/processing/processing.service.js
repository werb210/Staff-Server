...
export async function createDocumentProcessingJob(applicationId, documentId) {
    ...
    try {
       ...
       where application_id = $1 and document_id = $2`, [applicationId, documentId]);
        ...
        if (existingRecord) {
            if (config.flags.retryPolicyEnabled &&
       ...
       returning id, application_id, document_id, status, created_at, completed_at`, [randomUUID(), applicationId, documentId]);
        await advanceProcessingStage({ applicationId, client });
        await client.runQuery("commit");
...
export async function markDocumentProcessingCompleted(applicationId) {
    const client = await pool.connect();
    try {
        await client.runQuery("begin");
        const existing = await client.runQuery(`select id, application_id, document_id, status, created_at, completed_at
...
export async function markDocumentProcessingFailed(applicationId) {
    const client = await pool.connect();
    try {
        await client.runQuery("begin");
        const existing = await client.runQuery(`select id, application_id, document_id, status, created_at, completed_at
...
export async function createBankingAnalysisJob(applicationId) {
    ...
    try {
       ...
       values ($1, $2, 'pending')
       returning id, application_id, status, created_at, completed_at`, [randomUUID(), applicationId]);
        await advanceProcessingStage({ applicationId, client });
        await client.runQuery("commit");
        const insertedRecord = inserted.rows[0];
        if (!insertedRecord) {
...
export async function markBankingAnalysisCompleted(applicationId) {
    const client = await pool.connect();
    try {
        await client.runQuery("begin");
        const existing = await client.runQuery(`select id, application_id, status, created_at, completed_at
...
export async function markBankingAnalysisFailed(applicationId) {
    const client = await pool.connect();
    try {
        await client.runQuery("begin");
        const existing = await client.runQuery(`select id, application_id, status, created_at, completed_at
