...
async function insertReplayEvents(replayJobId, scope, sourceIds) {
    ...
    await db_1.pool.runQuery(`insert into ops_replay_events
...
async function createReplayJob(scope) {
    ...
    await db_1.pool.runQuery(`insert into ops_replay_jobs (id, scope, started_at, completed_at, status)
     values ($1, $2, null, null, 'queued')`, [id, scope]);
    return { id, scope, status: "queued" };
}
async function fetchReplayJobStatus(id) {
    const result = await db_1.pool.runQuery(`select id, scope, status, started_at, completed_at
...
async function listActiveReplayJobs() {
    const result = await db_1.pool.runQuery(`select id, scope, status, started_at
     from ops_replay_jobs
...
async function runReplayJob(id, scope) {
    ...
    await db_1.pool.runQuery(`update ops_replay_jobs
