...
async function upsertDailyMetricsWindow(params) {
    ...
    if (process.env.NODE_ENV === "test" && process.env.DATABASE_URL === "pg-mem") {
        const [applicationsCreated, applicationsSubmitted, applicationsApproved, applicationsDeclined, applicationsFunded, documentsUploaded, documentsApproved, lenderSubmissions,] = await Promise.all([
            runner.query("select count(*) from applications where created_at >= $1 and created_at < $2", [params.start, params.end]),
            runner.query("select count(*) from applications where pipeline_state = 'LENDER_SUBMITTED' and updated_at >= $1 and updated_at < $2", [params.start, params.end]),
            runner.query("select count(*) from applications where pipeline_state = 'APPROVED' and updated_at >= $1 and updated_at < $2", [params.start, params.end]),
            runner.query("select count(*) from applications where pipeline_state = 'DECLINED' and updated_at >= $1 and updated_at < $2", [params.start, params.end]),
            runner.query("select count(*) from applications where pipeline_state = 'FUNDED' and updated_at >= $1 and updated_at < $2", [params.start, params.end]),
            runner.query("select count(*) from document_versions where created_at >= $1 and created_at < $2", [params.start, params.end]),
            runner.query("select count(*) from document_version_reviews where status = 'accepted' and reviewed_at >= $1 and reviewed_at < $2", [params.start, params.end]),
            runner.query("select count(*) from lender_submissions where created_at >= $1 and created_at < $2", [params.start, params.end]),
        ]);
...
async function upsertLenderFunnelWindow(params) {
    ...
    const result = await runner.query(`insert into reporting_lender_funnel_daily
     ...
     set submissions = excluded.submissions,
         approvals = excluded.approvals,
         funded = excluded.funded`, [params.start, params.end, params.createdAt]);
    return result.rowCount ?? 0;
}
