    ...
    await runQuery(`insert into users (
     ...
     on conflict (phone_number) do update
       set email = excluded.email,
           ...
           phone_verified = excluded.phone_verified`, [SEEDED_ADMIN_ID, SEEDED_ADMIN_EMAIL, phoneNumber, phoneNumber, ROLES.ADMIN]);
    return { id: SEEDED_ADMIN_ID, phoneNumber };
}
export async function seedSecondAdminUser() {
    ...
    await runQuery(`insert into users (
     ...
     on conflict (phone_number) do update
       set email = excluded.email,
           ...
           disabled = excluded.disabled,
           locked_until = excluded.locked_until,
           phone_verified = excluded.phone_verified`, [SEEDED_ADMIN2_ID, SEEDED_ADMIN2_EMAIL, phoneNumber, phoneNumber, ROLES.ADMIN]);
    return { id: SEEDED_ADMIN2_ID, phoneNumber };
}
export async function seedBaselineLenders() {
    const { rows: tableRows } = await runQuery(`select table_name
    ...
    const { rows } = await runQuery("select count(*)::int as count from lenders");
    if ((rows[0]?.count ?? 0) > 0) {
        return;
    }
    await runQuery(`insert into lenders
    ...
    await runQuery(`insert into lender_products
