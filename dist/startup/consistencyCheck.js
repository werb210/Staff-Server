...
async function hasTable(table) {
    const res = await runQuery(`select count(*)::int as count
...
export async function runStartupConsistencyCheck() {
    if (config.env === "test") {
        return;
    }
    try {
        ...
        if (!hasDocuments || !hasApplications || !hasDocumentApplicationId) {
            ...
            const orphanDocumentsCount = await runQuery(`select count(*)::int as count
         from documents d
         where not exists (
           select 1 from applications a where a.id = d.application_id
         )`);
            const orphanDocumentsSample = await runQuery(`select d.id, d.application_id
         from documents d
         where not exists (
           select 1 from applications a where a.id = d.application_id
         )
        ...
        if (!hasApplications || !hasUsers || !hasOwnerUserId) {
            ...
            const orphanApplicationsCount = await runQuery(`select count(*)::int as count
         from applications a
         where not exists (
           select 1 from users u where u.id = a.owner_user_id
         )`);
            const orphanApplicationsSample = await runQuery(`select a.id, a.owner_user_id
         from applications a
         where not exists (
           select 1 from users u where u.id = a.owner_user_id
         )
        ...
        if (!hasApplications || !hasPipelineState) {
            ...
            const invalidPipelineCount = await runQuery(`select count(*)::int as count
         from applications
         where not (pipeline_state = any($1::text[]))`, [PIPELINE_STATES]);
            const invalidPipelineSample = await runQuery(`select id, pipeline_state
         from applications
         where not (pipeline_state = any($1::text[]))
         limit 10`, [PIPELINE_STATES]);
            await logCheckResult({
