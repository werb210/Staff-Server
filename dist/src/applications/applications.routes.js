...
export function createApplicationsRouter(controller = new ApplicationsController()) {
    ...
    router.post("/:id/credit-summary/regenerate", requireAuth, controller.regenerateCreditSummary);
    router.post("/:id/owners", requireAuth, controller.addOwner);
    router.put("/:id/owners/:ownerId", requireAuth, controller.updateOwner);
    router.delete("/:id/owners/:ownerId", requireAuth, controller.deleteOwner);
    return router;
}
