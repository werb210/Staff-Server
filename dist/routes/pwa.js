...
import { listPwaNotificationsForUser, listPwaSubscriptions, upsertPwaSubscription, deletePwaSubscription, acknowledgePwaNotification, deletePwaSubscriptionLegacy, } from "../repositories/pwa.repo.js";
...
router.post("/subscribe", requireAuth, requireAuthorization({ roles: ALL_ROLES }), safeHandler(async (req, res, next) => {
...
router.delete("/unsubscribe", requireAuth, requireAuthorization({ roles: ALL_ROLES }), safeHandler(async (req, res, next) => {
...
router.delete("/unsubscribe/owned", requireAuth, requireAuthorization({ roles: ALL_ROLES }), safeHandler(async (req, res, next) => {
    const parsedResult = unsubscribeSchema.safeParse(req.body ?? {});
    if (!parsedResult.success) {
        throw new AppError("validation_error", "Invalid unsubscribe payload.", 400);
    }
...
router.get("/subscriptions", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN] }), safeHandler(async (_req, res) => {
    const subscriptions = await listPwaSubscriptions();
    res.status(200).json({ ok: true, subscriptions });
}));
router.get("/notifications", requireAuth, requireAuthorization({ roles: ALL_ROLES }), perUserNotificationReadLimiter, safeHandler(async (req, res, next) => {
...
router.post("/notifications/:id/ack", requireAuth, requireAuthorization({ roles: ALL_ROLES }), perUserNotificationAckLimiter, safeHandler(async (req, res, next) => {
...
router.post("/sync", requireAuth, requireAuthorization({ roles: ALL_ROLES }), safeHandler(async (req, res, next) => {
    const requestId = res.locals.requestId ?? "unknown";
    const user = req.user;
    const replayUser = {
        userId: user.userId,
...
router.get("/runtime", requireAuth, requireAuthorization({ roles: ALL_ROLES }), safeHandler(async (_req, res) => {
...
router.get("/health", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN] }), safeHandler(async (_req, res) => {
    const pushStatus = fetchPushStatus();
    ...
    try {
        const tableCheck = await runQuery("select to_regclass('public.ops_replay_jobs') as exists");
        if (!tableCheck.rows[0]?.exists) {
            queueProcessingHealthy = null;
            warnings.push("queue_table_missing");
        }
        else {
            const result = await runQuery(`select count(*)::int as count
