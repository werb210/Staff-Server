...
const router = Router();
router.get("/lender-products", safeHandler(async (_req, res) => {
    const products = await listLenderProducts();
    res.status(200).json(products.map((p) => ({ id: p.id, name: p.name, type: p.category })));
}));
router.get("/lender-products/:id/requirements", safeHandler(async (req, res, next) => {
...
router.get("/lender-products/requirements", safeHandler(async (req, res, next) => {
