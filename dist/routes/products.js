...
router.get("/", async (_req, res) => {
    try {
        const result = await db.query(`select id, lender_id, name, category, status, terms_summary, created_at
...
router.get("/knowledge", async (_req, res) => {
    try {
        const docs = await db.query(`select id, source_type, source_id, content, created_at
       from ai_knowledge
