...
router.post("/log", safeHandler(async (req, res, next) => {
    const parsed = z
        .object({
...
router.post("/start", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN, ROLES.STAFF] }), safeHandler(async (req, res, next) => {
...
router.post("/:id/status", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN, ROLES.STAFF] }), safeHandler(async (req, res, next) => {
...
router.post("/:id/end", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN, ROLES.STAFF] }), safeHandler(async (req, res, next) => {
    const id = toStringSafe(req.params.id);
    if (!id || !uuidSchema.safeParse(id).success) {
        throw new AppError("validation_error", "Invalid call id.", 400);
    }
...
router.get("/", requireAuth, requireAuthorization({ roles: [ROLES.ADMIN, ROLES.STAFF] }), safeHandler(async (req, res, next) => {
    const contactId = typeof toStringSafe(req.query.contactId) === "string" ? toStringSafe(req.query.contactId) : null;
    const applicationId = typeof toStringSafe(req.query.applicationId) === "string" ? toStringSafe(req.query.applicationId) : null;
