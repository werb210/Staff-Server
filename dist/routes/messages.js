...
router.post("/", safeHandler(async (req, res, next) => {
    ...
    const id = randomUUID();
    await runQuery(`insert into communications_messages (id, type, direction, status, contact_id, body, created_at)
       values ($1, 'message', coalesce($2, 'inbound'), 'received', null, $3, now())`, [id, optionalString(req.body?.direction) ?? "inbound", body]);
