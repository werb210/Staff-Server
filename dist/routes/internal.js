...
router.post("/bootstrap-admin", wrap(async (req) => {
    logInfo("bootstrap_admin_attempt", {
        disabled: bootstrapAdminDisabled,
    });
    if (bootstrapAdminDisabled) {
        throw new AppError("bootstrap_disabled", "Bootstrap has already been used.", 410);
    }
    const phoneNumber = config.bootstrap.adminPhone;
    if (!phoneNumber) {
        throw new AppError("bootstrap_missing_phone", "BOOTSTRAP_ADMIN_PHONE is required.", 500);
    }
    const countRes = await runQuery("select count(*)::int as count from users where role = $1", [ROLES.ADMIN]);
    ...
    if (adminCount > 0) {
        ...
        throw new AppError("bootstrap_disabled", "Admin user already exists.", 409);
    }
...
router.get("/failed-jobs", wrap(async () => {
    const result = await runQuery(`SELECT id, type, error, retry_count, created_at
       FROM failed_jobs
