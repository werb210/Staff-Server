...
exports.pool = exports.isPgMem = void 0;
exports.checkDb = checkDb;
exports.assertSchema = assertSchema;
const pg_1 = require("pg");
exports.isPgMem = process.env.NODE_ENV === "test" &&
...
async function assertSchema() {
    const res = await exports.pool.query(`select table_name, column_name
     from information_schema.columns
     where table_schema = 'public'`);
    const existing = new Set(res.rows.map((row) => `${row.table_name}.${row.column_name}`));
    const missing = requiredColumns.filter((item) => !existing.has(`${item.table}.${item.column}`));
    if (missing.length > 0) {
