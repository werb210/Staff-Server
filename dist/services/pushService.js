...
function ensurePayloadSize(payload) {
    const maxBytes = (0, config_1.getPwaPushPayloadMaxBytes)();
...
function getAuditEntry(payload) {
    if (payload.type === "alert") {
        return {
            level: payload.level,
            title: payload.title,
...
function initializePushService() {
    ...
    const publicKey = (0, config_1.getVapidPublicKey)();
    const privateKey = (0, config_1.getVapidPrivateKey)();
    const subject = (0, config_1.getVapidSubject)();
...
async function sendNotification(target, payload) {
    ...
    const requestId = (0, requestContext_1.getRequestContext)()?.requestId ?? "unknown";
