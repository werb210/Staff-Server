...
async function uploadDocumentIdempotent(input) {
    const existing = await db_1.pool.runQuery(`select id, application_id, document_type as category, filename, storage_key as "storageKey", 0::int as size
     from documents
    ...
    const created = await db_1.pool.runQuery(`insert into documents (id, application_id, owner_user_id, title, document_type, status, filename, storage_key, uploaded_by, created_at, updated_at)
