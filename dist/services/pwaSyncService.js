...
function requireRequiredDocuments(value) {
    ...
    for (const entry of value) {
        if (typeof entry === "string") {
            ...
            if (!normalizedKey) {
                throw new AppError("validation_error", "required_documents contains invalid keys.", 400);
            }
        ...
        throw new AppError("validation_error", "required_documents must contain strings or objects.", 400);
    }
...
function normalizeVariableRateString(value, fieldName) {
    if (!value || value.trim().length === 0) {
        throw new AppError("validation_error", `${fieldName} is required for VARIABLE rates.`, 400);
    }
    ...
    throw new AppError("validation_error", `${fieldName} must be a number or Prime + X.`, 400);
}
...
function assertActionPayloadSize(body) {
    ...
    if (size > maxBytes) {
        throw new AppError("payload_too_large", `Replay payload exceeds ${maxBytes} bytes.`, 413);
    }
}
...
function assertCapabilities(user, required) {
    ...
    if (!hasAll) {
        throw new AppError("insufficient_capabilities", "User lacks required capabilities.", 403);
    }
}
async function executeReplayAction(action, user) {
    ...
    if (action.path === "/api/lender-products") {
        ...
        if (!isLenderActive(lender)) {
            throw new AppError("lender_inactive", "Lender must be active to add products.", 400);
        }
...
export async function replaySyncBatch(params) {
    ...
    if (parsed.actions.length > maxActions) {
        throw new AppError("too_many_actions", `Replay batch exceeds ${maxActions} actions.`, 413);
    }
    ...
    if (batchSize > maxBatchBytes) {
        throw new AppError("payload_too_large", `Replay batch exceeds ${maxBatchBytes} bytes.`, 413);
    }
    ...
    for (const action of parsed.actions) {
        ...
        try {
            if (!action.idempotencyKey || action.idempotencyKey.length > 128) {
                throw new AppError("missing_idempotency_key", "Idempotency-Key is required.", 400);
            }
            if (containsJwt(action.body)) {
                throw new AppError("jwt_not_allowed", "JWT tokens are not allowed in replay payloads.", 400);
            }
            ...
            if (existing) {
                if (existing.request_hash !== requestHash) {
                    throw new AppError("idempotency_conflict", "Idempotency key reused with different payload.", 409);
                }
                results.push({
                    id: action.id,
                    status: "succeeded",
